    ...
    if (target && target.getAttribute("href")) {
      ...
      if (domain_matches && !consent_granted_or_not_needed) {
        ...
        if (na_temp && typeof na_temp === 'object') {
          ...
          if (enable_logs) {
            console.log('cross-domain > Consent denied. Decorating URL with na_temp params:', na_temp);
          }
        } else {
          if (enable_logs) {
            console.log('cross-domain > Consent denied. Cookie na_temp not found or invalid, no decoration applied.');
          }
        }
      
        if (enable_logs) {
          console.log('cross-domain >   👉 Redirect to: ' + link_url.href);
        }
